...
class DDPG(object):
	def __init__(self, actor, critic, difference_critic, memory, observation_shape, action_shape,
		use_difference_critic=False, gamma=0.99, tau=0.001, normalize_returns=False, normalize_observations=True,
		batch_size=128, observation_range=(-5., 5.), action_range=(-1., 1.), return_range=(-np.inf, np.inf),
		critic_l2_reg=0., actor_lr=1e-4, difference_critic_lr=1e-4, critic_lr=1e-3, clip_norm=None, reward_scale=1.):
		...
		self.difference_critic_optimizer = tf.train.AdamOptimizer(learning_rate=difference_critic_lr)
	...
	def train(self, global_step):
		if self.use_difference_critic:
			...
			actionA1s = tf.constant(self.actor(batchA['obs1']))
			actionB1s = tf.constant(self.actor(batchB['obs1']))
			diff_Qs = self.difference_critic(tf.constant(batchA['obs1']), actionA1s, tf.constant(batchB['obs1']), actionB1s)
			target_diff_Qs = batchA['rewards'] - batchB['rewards'] + self.gamma * diff_Qs
			target_diff_Qs = tf.clip_by_value(target_diff_Qs, 2*self.return_range[0], 2*self.return_range[1])
			...
			difference_critic_grad = diff_critic_grad_fn(batchA['obs0'], batchA['actions'], 
				batchB['obs0'], batchB['actions'], target_diff_Qs)
			...
			actor_grad = actor_grad_fn(batchA['obs0'], batchB['obs0'], batchB['actions'])
			...
			# Prepare training data for critic
			action1s = self.actor(batch['obs1'])
			Qs = self.critic(batch['obs1'], action1s)
			terminals = batch['terminals1'].astype('float32')
			target_Qs = batch['rewards'] + (1. - terminals) * self.gamma * Qs
			target_Qs = tf.clip_by_value(target_Qs, self.return_range[0], self.return_range[1])
			...
			critic_grad = critic_grad_fn(batch['obs0'], batch['actions'], target_Qs)
			...
			actor_grad = actor_grad_fn(batch['obs0'])
	...
	def get_variables_to_save(self, global_step):
		return (self.actor.variables + self.critic.variables
			+ self.difference_critic.variables 
			+ self.actor_optimizer.variables() + self.critic_optimizer.variables() 
